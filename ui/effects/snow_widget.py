...
class Snowflake:
    ...
    def __init__(self, x: float, y: float, size: float, speed: float):
        ...
        self.size = size
        self.speed = speed
        self.drift = random.uniform(-0.5, 0.5)  # Горизонтальное покачивание
...
class SnowWidget(BaseWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.setStyleSheet("background: transparent;")  # Оригинальный стиль
    ...
    def init_snowflakes(self) -> None:
        ...
        for _ in range(30):
            ...
            size = random.randint(3, 8)  # Оригинальные размеры
            speed = random.uniform(1, 2.5)  # Оригинальная скорость
    ...
    def reinit_snowflakes_for_size(self, new_width: int, new_height: int) -> None:
        ...
        while len(self.snowflakes) < target_count:
            ...
            y = random.randint(-new_height, new_height)  # Оригинальный диапазон
        ...
        # Корректируем позиции существующих снежинок
    ...
    def update_snowflakes(self) -> None:
        ...
        for snowflake in self.snowflakes:
            ...
            snowflake.x += snowflake.drift  # Используем drift вместо random
            
            # Сброс снежинки если она вышла за границы
            ...
            # Оригинальная логика для боковых границ
            if snowflake.x < -20:
    ...
    def paintEvent(self, event) -> None:
        ...
        for snowflake in self.snowflakes:
            color = QColor(255, 255, 255, int(snowflake.opacity * 128))  # Оригинальная прозрачность
            ...
            # Лучи снежинки (оригинальная отрисовка)
            painter.setPen(QPen(color, 2))
            painter.drawLine(x - size, y, x + size, y)  # Горизонтальная линия
            painter.drawLine(x, y - size, x, y + size)  # Вертикальная линия
